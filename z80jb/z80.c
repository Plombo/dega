 ...
 ***************************************************************/
#define IN(port)   ((uint8_t)Z80IORead(port))
 ...
 ***************************************************************/
#define RM(addr) (uint8_t)Z80ProgramRead(addr)

/***************************************************************
 * Read a word from given memory location
 ***************************************************************/
Z80_INLINE void RM16( uint32_t addr, Z80_PAIR *r )
{
	r->b.l = RM(addr);
 ...
 ***************************************************************/
Z80_INLINE void WM16( uint32_t addr, Z80_PAIR *r )
{
	WM(addr,r->b.l);
...
Z80_INLINE uint32_t ARG16(void)
{
	unsigned pc = PCD;
	PC += 2;
 ...
 ***************************************************************/
#define EAX EA = (uint32_t)(uint16_t)(IX + (int8_t)ARG())
#define EAY EA = (uint32_t)(uint16_t)(IY + (int8_t)ARG())
...
static void z80_set_info(uint32_t state, cpuinfo *info)
{
	switch (state)
	{
...
void z80_get_info(uint32_t state, cpuinfo *info)
{
	switch (state)
	{
